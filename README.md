...
## KV Cache Policy 统一接口

KVCache-Serve 提供可插拔的 KV Cache Policy 抽象，用于统一接入不同的缓存保留、裁剪和预算策略。

核心模块：

    app/kvcache/policy.py
    app/kvcache/registry.py
    app/kvcache/noop_policy.py

统一接口包含：

- `KVCachePolicyContext`：描述请求、Prefill/Decode 阶段、Decode 步数和当前缓存 Token 数；
- `KVCachePolicyResult`：返回处理后的 `past_key_values` 以及策略统计信息；
- `KVCachePolicy`：所有缓存策略必须实现的抽象接口；
- `KVCachePolicyRegistry`：负责策略注册、名称规范化和实例创建；
- `NoOpKVCachePolicy`：不修改 KV Cache 的基线策略。
...
`noop` 策略不会修改 `past_key_values`，用于验证 Policy 抽象不会改变原有推理结果和缓存行为。

同步请求示例：

    curl -X POST http://localhost:18000/chat       -H "Content-Type: application/json"       -d '{
...
异步请求示例：
    ...
    curl -X POST http://localhost:18000/queue/chat       -H "Content-Type: application/json"       -d '{
        "prompt": "Kubernetes KV Cache Policy",
        "model": "local-llm",
        "max_tokens": 4,
        "kv_policy": "noop",
        "kv_policy_config": {}
...
- `kv_policy`：实际使用的策略名称；
- `kv_policy_applied_count`：策略被应用的总次数；
- `kv_policy_tokens_before`：最近一次应用策略前的缓存 Token 数；
- `kv_policy_tokens_after`：最近一次应用策略后的缓存 Token 数；
- `kv_policy_evicted_tokens`：本次请求累计淘汰的缓存 Token 数。

真实 tiny-gpt2 NoOp 验证结果：

    kv_policy=noop
    kv_policy_applied_count=4
    kv_policy_tokens_before=13
    kv_policy_tokens_after=13
    kv_policy_evicted_tokens=0
    kv_cache_tokens=13

Runtime 会记录最近一次 Policy 事件：
...
未注册的策略会在同步或异步入口直接返回 HTTP 400，避免无效任务进入 Redis 队列。

该接口是后续 Full Cache、Sliding Window 和 Fixed Budget 策略的统一扩展基础。新增策略只需实现 `KVCachePolicy.apply()` 并注册到 Registry，无需修改 API 和 Worker 主流程。
